...
class AuthService:
    ...
    async def get_token(self, data, db):
        ...
        return await self.get_user_token(user=user)

    async def get_refresh_token(self, token: str, db):
        ...
        refresh_token_expiry = timedelta(minutes=self.settings.REFRESH_TOKEN_EXPIRE_MINUTES)
        ...
        return await self.get_user_token(user=user, refresh_token=token)

    async def get_user_token(self, user: UserModel, refresh_token=None):
        payload = {"id": user.id}
        access_token_expiry = timedelta(minutes=self.settings.ACCESS_TOKEN_EXPIRE_MINUTES)
        refresh_token_expiry = timedelta(minutes=self.settings.REFRESH_TOKEN_EXPIRE_MINUTES)
